...
export function MoatDashboard() {
  ...
  const topSymptoms = ontology.sort((a, b) => b.selectedCount - a.selectedCount).slice(0, 5);
    ...
    <main className="mx-auto max-w-6xl px-4 pb-28 pt-6 md:px-8">
      <div className="rounded-[2rem] border border-amber/30 bg-soft-amber p-5">
        <p className="flex items-center gap-2 text-sm font-semibold text-amber">
          <LockKeyhole size={18} /> Internal Workflow Intelligence — Demo Data
        </p>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-3">
        {[
          ...
          ["Nurse delight score", feedback.length ? `${Math.round(feedback.reduce((sum, item) => sum + item.timeSavingScore, 0) / feedback.length)}/5` : "pending"],
