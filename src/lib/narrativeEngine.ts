...
function buildSections(input: NarrativeInput): Record<string, string> {
  ...
  return {
    ...
    assessmentDraft: providerDraftAllowed(input)
      ? "Assessment draft: clinical impression placeholder for licensed clinician review only."
...
export function generateNarrative(input: NarrativeInput): NarrativeOutput {
  ...
  if (mode === "nursing") {
    text = compact([
      ...
      sections.reassessment,
      sections.providerNotification,
      "Symptoms documented for provider evaluation."
    ]);
  }
  ...
  if (mode === "soap") {
    ...
    if (providerDraftAllowed(input)) {
      soapSections.push(`Assessment draft: ${sections.assessmentDraft}`);
      soapSections.push(`Plan draft: ${sections.planDraft}`);
    } else {
      soapSections.push("Assessment/Plan: not generated for selected role. Provider evaluation requested.");
    }
  ...
  if (mode === "handoff") {
    text = [
      ...
      `Pending follow-up: ${sections.disposition}`,
      `Safety concerns: ${input.selectedNegatives.length ? `Pertinent negatives documented: ${list(input.selectedNegatives)}.` : "No safety concerns selected."}`
    ].join("\n");
  }
  ...
  if (mode === "triage") {
    text = compact([
      ...
      selectedSymptomLabels(input).length ? `Associated symptoms: ${list(selectedSymptomLabels(input))}.` : false,
      input.complaintGroup.redFlags.length ? `Red flags considered: ${list(input.complaintGroup.redFlags)}.` : false,
      input.selectedNegatives.length ? `Red flags denied/present by selection: denies ${list(input.selectedNegatives)}.` : false,
      observations && `Acuity cues: ${observations}`,
      "No final diagnosis assigned. Symptoms documented for provider evaluation."
    ]);
  }
  ...
  if (mode === "telehealth") {
    text = [
      ...
      `Remote encounter limitations: physical exam limited to patient-reported history and observable video/phone cues documented in this session.`,
      `Clinician observations: ${observations || "No remote observations selected."}`,
      `Follow-up instructions placeholder: reviewing clinician to complete.`,
      `Escalation precautions placeholder: reviewing clinician to complete.`,
      "Clinician review required."
    ].join("\n");
  }
